// SHELVY_ESP32_v6_cloud.ino
// Bread storage chamber controller — v6 (copy of v5; ready for a proper re-calibration).
//
// WHAT CHANGED vs v5:
//   - Same firmware. The ONLY thing to change here is the two calibration offsets,
//     once you have a TRUSTWORTHY reference (the FY-12 is broken — do NOT calibrate to it).
//   - Calibrate temperature with ICE WATER = 0.0 C (physics-guaranteed): seal the sensor
//     in a dry bag, put it in stirred ice+water, read the ESP32, set TEMP_CAL_OFFSET so it
//     reads 0.0 C. Or use a NEW working thermometer placed right next to the ESP32 sensor.
//
// HARDWARE
...
//   Power the ESP32 from CLEAN power (Pi USB / laptop / power bank), NOT the 12V PSU.
...
// ============================================================
// 2. DFR0588 CONVERSION (fixed 0.3-2.7V window)
// ============================================================
...
// >>> CALIBRATION — carried over from v5 (temp validated at 31.6C & 27.2C on a WORKING FY-12).
//     RE-CHECK with a trustworthy reference (ice water = 0C, or a new thermometer beside the
//     sensor) and adjust ONLY these two numbers, then re-upload. Do NOT tune to the broken FY-12.
const float TEMP_CAL_OFFSET = -2.5f;   // add to temperature reading
const float HUMI_CAL_OFFSET = 8.0f;    // add to humidity reading (superseded — use SHELVY_ESP32_FINAL2, offset 6.75)
...
// 3. CONTROL SETPOINTS
// ============================================================
// Relay logic (temperature only; humidity is monitored, not controlled):
//   T >= PULLDOWN  -> both Peltiers ON
//   STOP < T < PULLDOWN -> one Peltier, alternating (MAINTAIN, holds ~SETPOINT)
//   T <= STOP      -> both OFF (IDLE)
const float TEMP_SETPOINT_C = 25.0;
const float TEMP_PULLDOWN_C = 26.0;
const float TEMP_STOP_C     = 24.0;   // <-- change to 25.0 if you want it to switch OFF at 25C
