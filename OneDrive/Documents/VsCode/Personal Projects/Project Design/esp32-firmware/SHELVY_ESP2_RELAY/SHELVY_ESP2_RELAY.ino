...
//   - Runs the 27.5C + 30s-off control, drives the relay
//   - Clicks 6x on boot (self-test) so you HEAR the relay works
//   - Has NO sensor -> no analog pins to corrupt
//
// The temp comes WIRELESSLY from ESP1 -> no wire between the ESP32s -> the
// noisy 12V ground here can NEVER reach ESP1's clean sensor.
//
// WIRING:
//   Relay:  IN1->GPIO32  IN2->GPIO33   DC+ ->12V(+)  DC- ->12V(-)
//           high/low jumper: S1->Low, S2->Low   (active-LOW)
//           load on COM + NC (cooling ON by default; opens/OFF at 27.5C)
//   Power:  PSU 12V -> Buck(5V) -> ESP32
...
const uint32_t RELAY_OFF_MS    = 30000;   // 30s off when 27.5C reached
...
void relaySelfTest(){
  ...
  for(int c=0;c<6;c++){
    digitalWrite(RELAY_PIN[0],LOW); digitalWrite(RELAY_PIN[1],LOW);
    Serial.printf("[SELFTEST] click %d - ENERGIZED\n", c+1); delay(800);
    digitalWrite(RELAY_PIN[0],HIGH); digitalWrite(RELAY_PIN[1],HIGH);
...
void setup(){
  ...
  relayInit();
  // relaySelfTest();   // disabled: no boot power-cut. cooling flows immediately on NC.   // clicks 6x on boot so you HEAR it
  WiFi.mode(WIFI_STA);
  ...
  Serial.println("[ESP2] RELAY NODE - fetches temp from backend, drives relay");
}
