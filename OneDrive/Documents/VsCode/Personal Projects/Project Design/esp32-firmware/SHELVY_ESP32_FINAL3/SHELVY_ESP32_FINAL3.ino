// SHELVY_ESP32_FINAL3.ino
// Bread storage chamber controller — FINAL 3 / v3 (2026-09-23, validation day).
//
// WHAT CHANGED vs FINAL2 (v3):
//   1. RELAY WIRING = OPTO-ISOLATED. The relay board's yellow VCC<->JD-VCC jumper
//      is REMOVED, so the 12V coil ground NEVER touches the ESP32 ground. This
//      kills the ground-noise that was spiking the analog sensor to ~59C whenever
//      the grounds were bonded. Sensor stays clean AND the relay switches.
//         Clean side (from ESP32/power bank):  VCC<-5V  GND<-GND  IN1/IN2<-GPIO
//         Dirty side (from 12V PSU):           JD-VCC<-12V(+)     coilGND<-COM
//         RULE: ESP32 GND and 12V COM are NEVER wired together (opto bridges them).
//   2. RELAY_NC = { false, false }. Peltiers are wired to COM + NO (verified by
//      continuity on validation day: NO = RIGHT terminal, NC = LEFT terminal).
//      => Peltier OFF at rest, ON when the relay coil energises (correct direction).
//   3. ESP32 powered from the 5V power bank via Type-C (NO buck converter — the
//      buck was the original sensor-noise source).
//
// CALIBRATION (unchanged from FINAL2, synced to FY reference at operating point):
//   TEMP_CAL_OFFSET = -2.8   (SHT30 26.3C -> FY 26.0C)
//   HUMI_CAL_OFFSET =  1.0   (app was ~73% vs FY ~70% at the cold operating point)
//
// CONTROL (unchanged from FINAL2):
//   PULLDOWN 25C = both ON above 25C; at 25C one system turns OFF (first switch).
//   MAINTAIN 23-25C = one system alternates (fluctuation active at 24C).
//   STOP 23C = both OFF only at the 23C floor (rarely reached).
//
// HARDWARE
//   Board  : ESP32 Dev Module (esp32 core 3.3.11), on 38-pin expansion board
//   Power  : 5V power bank -> ESP32 Type-C (no buck)
//   Sensor : DFRobot DFR0588 Gravity ANALOG SHT30
//            GND->GND · VCC->3V3 · T->GPIO34 · RH->GPIO35
//   Relay  : opto-isolated 4-ch (jumper OFF). IN1<-GPIO33  IN2<-GPIO26
//            VCC<-5V  GND<-GND (clean) | JD-VCC<-12V  coilGND<-COM (dirty)
//            Peltier on COM + NO (RIGHT terminal).
...
// FINAL 3 calibration vs FY reference (synced at the cold operating point):
const float TEMP_CAL_OFFSET = -2.8f;   // add to temperature reading (SHT30 26.3C -> FY 26.0C)
const float HUMI_CAL_OFFSET = 1.0f;    // add to humidity reading (app ~73% was too high vs FY ~70%)
