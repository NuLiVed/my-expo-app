// SHELVY_ESP32_FINALACT3.ino
// Same as FINALACT2: RELAY CLICK SELF-TEST on boot, then 27.5C control.
//
...
//
// WIRING:
//   Relay:  IN1->GPIO32  IN2->GPIO33
//           DC+ -> 12V(+)   DC- -> 12V(-)   (single-supply high/low board)
//           high/low jumper: S1->Low, S2->Low   (active-LOW)
//           load on COM + NC (cooling ON by default)
//   Power:  PSU 12V -> Buck(5V) -> ESP32   (buck ties ESP32 gnd to 12V gnd)
//   Sensor: DFR0588 analog SHT30 — VCC->3V3(LDO pin)  T->GPIO34  RH->GPIO35  GND->GND
...
const float TEMP_CAL_OFFSET = -2.8f;   // reverted to original working offset (calibrated to FY at operating point)
...
void TaskSensors(void* pv) {
  ...
  for (;;) {
    ...
    if (ok) {
      ...
      Serial.printf("[SENSOR] out of range vT=%.3f vH=%.3f (fail %d) - check VCC=3V3, T=34, RH=35\n", vT, vH, g_sensorFailCount);
    }
